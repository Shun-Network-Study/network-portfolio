# ネットワーク構築演習ポートフォリオ

## 自己紹介・ポートフォリオの概要
前職は映像ディレクターとして従事しておりましたが、より⾧期的な成⾧と社会的な安定を求めてIT業界へのキャリアチェンジを目指し、未経験からネットワークエンジニアとしてのキャリアを志望しております。<br>
重要な社会基盤を支えるネットワーク技術に強い関心を持ち将来性を感じて、2025年5月にCCNA（Cisco Certified Network Associate）を取得し、ネットワーク構築スキルを高めるために、Cisco Packet TracerとCisco Modeling Labsを使ってハンズオン学習を続けています。

本リポジトリは、ネットワークエンジニアとしての実践力を高めるために取り組んだハンズオン学習の記録です。「ネットワーク構築マラソン」（株式会社CAIRN 運営）に取り組み、ネットワーク構成図、設定内容、検証結果などを体系的にまとめています。
実務未経験ではありますが、実務を想定した構築課題に取り組み、設定ミスの切り分けや通信確認などのトラブルシューティングの思考を可視化することで、ポートフォリオを通じて、採用担当者の方にスキルと姿勢をお伝えできればと存じます。

## ハンズオン実践記録
「ネットワーク構築マラソン」（株式会社CAIRN 運営）<br>
クリックすると閲覧できます
- [第1回：Cisco機器の基本設定](/study01/README.md)
- ![構成図](/study01/images/topology1.png)
- [第2回：ルーティングの設定](/study02/README.md)
- [第3回：アクセスリスト(ACL)の設定](/study03/README.md)
- [第4回①：NATの設定(基本)](/study04①/README.md)
- [第4回②：NATの設定(応用)](/study04②/README.md)
- [第5回：DHCPの設定](/study05/README.md)
- [第6回：VLANの設定(基本)](/study06/README.md)

## 検証環境
- Cisco Packet Tracer / Cisco Modeling Labs

## 保有資格
- CCNA（Cisco Certified Network Associate） 【2025年5月8日 取得】

## 技術スキル
- **Cisco機器**: Layer2 Switch・Router
- **ネットワーク基礎**: OSI参照モデル・TCP/IP
- **スイッチング**: VLAN・STP・EtherChannel
- **ルーティング**: スタティックルーティング・OSPF
- **セキュリティ**: ACL・NAT（スタティックNAT・ダイナミックNAT・NAPT）

## 学習履歴
- **学習期間**: 2024年10月〜2025年5月
- **Webサービス**: 
  - Ping-t 最強WEB問題集 1633問
  - Ping-t コマンド問題 171問 （全5周実施）
  - ウズカレテスト（株式会社UZUZ COLLEGE 運営）コマンド問題 149問 （全5周実施）
- **書籍**: 
  - シスコ技術者認定教科書 CCNA 完全合格テキスト&問題集［対応試験］200-301 （全3周実施）
  - 徹底攻略Cisco CCNA問題集［200-301 CCNA］対応 （全3周実施）
  - キタミ式イラストIT塾 基本情報技術者 令和06年 
- **Cisco実機シミュレーター**: Cisco Packet Tracer・Cisco Modeling Labs
