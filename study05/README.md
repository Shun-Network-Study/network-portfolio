...
### 検証用PC（PC A・PC B）の設定
#### 検証用PC（PC A・PC B）にIPアドレスを自動的に取得するためにデフォルトの設定の『Static』から『DHCP』にチェックを入れる
![検証用PC（PC A）『DHCP』にチェックを入れる](/study05/images/課題5_1.png)

- 『DHCP』にチェックを入れた後、「DHCP failed. APIPA is being used.」と表示された
- CCNA白本で調査したところ、以下のことが判明
- APIPA（Automatic Private IP Addressing）: DHCPを利用する環境で、DHCPが機能せずにIPアドレスが割り当てられない時に自動的にIPアドレスを割り当てる技術
- APIPAで割り当てられるIPアドレスの範囲: 169.254.0.0 ～ 169.254.255.255　サブネットマスク: 255.255.0.0
- DHCPが利用できない場合、APIPA（Automatic Private IP Addressing）によって自動的にIPアドレスが割り当てられる

![検証用PC（PC B）『DHCP』にチェックを入れる](/study05/images/課題5_2.png)
...
### 検証用PC（PC A・PC B）に対して払い出されたIPアドレスをコマンドプロンプトで確認
...
**技術的な学び**
- 『DHCP』にチェックを入れた後に「DHCP failed. APIPA is being used.」と表示された
- CCNA白本で調査した所、以下のことが判明
- APIPA（Automatic Private IP Addressing）: DHCPを利用する環境で、DHCPが機能せずにIPアドレスが割り当てられない時に自動的にIPアドレスを割り当てる技術
- APIPAで割り当てられるIPアドレスの範囲: 169.254.0.0 ～ 169.254.255.255　サブネットマスク: 255.255.0.0
- DHCPが利用できない場合、APIPA（Automatic Private IP Addressing）によって自動的にIPアドレスが割り当てられる
...
**トラブルシューティング**
- APIPAアドレス（169.254.x.x）が割り当てられた場合は、DHCPサーバの設定を再確認
- service dhcpが有効になっているか、IPアドレス・デフォルトゲートウェイの設定に誤りがないか確認する必要がある

**設定コマンドの学び**
- (dhcp-config)#network [ネットワークアドレス] [サブネットマスク] → IPアドレスの範囲を開始・終了のような指定ではなく、ネットワークアドレスとサブネットマスクを指定する
- (dhcp-config)#default-router [デフォルトゲートウェイのIPアドレス] → 割り当てるデフォルトゲートウェイを指定
- #show running-config → DHCPサーバの設定内容を確認

**所感**
- DHCPの仕組みを設定から検証まで一通り体験できたことで、IPアドレスの自動割り当ての利便性やトラブル時の対処法を学ぶことができた
- APIPAの動作を確認することで、DHCPが機能しない場合の代替手段についても理解が深まった
- 今後は、より複雑なネットワーク環境でのDHCP運用や、トラブルシューティングの経験も積みたいと思った
